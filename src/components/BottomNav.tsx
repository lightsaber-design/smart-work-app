import { BarChart3, Settings, MapPin, CalendarDays, Clock } from "lucide-react";
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-card/90 backdrop-blur-xl border-t border-border safe-bottom">
      <div className="flex justify-around items-center h-16 max-w-md mx-auto px-4">
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 px-3 py-2 transition-colors ${activeTab === "map" ? "text-primary" : "text-muted-foreground"}`}
        >
          <MapPin className="w-5 h-5" />
          <span className="text-[10px] font-medium">{t("nav_map")}</span>
        </button>
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 px-3 py-2 transition-colors ${activeTab === "settings" ? "text-primary" : "text-muted-foreground"}`}
        >
